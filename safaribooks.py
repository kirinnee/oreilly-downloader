...
class SafariBooks:
    ...
    def check_login(self):
        # The orm-jwt token expires quickly, and O'Reilly serves TRUNCATED chapter
        # content once it has expired (HTTP 200, but only a preview), which would
        # silently produce a broken EPUB. Detect expiry up front from the token itself.
        jwt = self.session.cookies.get("orm-jwt")
        if jwt and "." in jwt:
            try:
                payload = jwt.split(".")[1]
            ...
            if exp and exp < time.time():
                self.display.exit(
                    "Authentication issue: your session token (orm-jwt) has EXPIRED.\n" +
                    self.display.SH_YELLOW + "[*]" + self.display.SH_DEFAULT +
                    " O'Reilly serves truncated content with an expired token, so the download\n"
                    "    would be incomplete. Please re-export fresh cookies from your browser\n"
                    "    (while logged in) and run again promptly.")

        # Verify the session against the profile page; academic/SSO accounts redirect
        # /profile/ to a login check even when valid, so fall back to the book API.
        response = self.requests_provider(PROFILE_URL, perform_redirect=False)
        if response == 0:
            self.display.exit("Login: unable to reach Safari Books Online. Try again...")

        if response.status_code == 200 and "user_type\":\"Expired\"" in response.text:
            self.display.exit("Authentication issue: account subscription expired.")

        if response.status_code != 200:
            api_check = self.requests_provider(
                self.API_TEMPLATE.format(self.args.bookid), perform_redirect=False)
            if api_check == 0 or api_check.status_code != 200:
                self.display.exit("Authentication issue: unable to access O'Reilly with these cookies.\n"
                                  "    Please re-export fresh cookies and try again.")
    ...
    def get_book_info(self):
        ...
        info = response.json()
        if not isinstance(info, dict) or len(info.keys()) == 1:
            self.display.exit(self.display.api_error(info))

        # --- v2 API adapter: map /api/v2/epubs/urn:orm:book:<id>/ to the fields ---
        # --- the rest of the program (originally written for /api/v1/book/) expects. ---
        description = ""
        if isinstance(info.get("descriptions"), dict):
            description = info["descriptions"].get("text/html") \
                          or info["descriptions"].get("text/plain") or ""

        book_info = {
            "title": info.get("title", self.book_id),
            "identifier": info.get("identifier", self.book_id),
            "web_url": SAFARI_BASE_URL + "/library/view/book/%s/" % self.book_id,
            "isbn": info.get("isbn") or self.book_id,
            "description": description,
            "issued": info.get("publication_date", ""),
            "authors": info.get("authors", []),
            "publishers": info.get("publishers", []),
            "subjects": info.get("subjects", []),
            "rights": info.get("rights", "") or "",
            "_chapters_url": info.get("chapters")
            or (SAFARI_BASE_URL + "/api/v2/epub-chapters/?epub_identifier=urn:orm:book:%s" % self.book_id),
            "_toc_url": info.get("table_of_contents") or urljoin(self.api_url, "table-of-contents/"),
        }
    ...
    def get_book_chapters(self, page=1):
        # v2 API: chapters live at /api/v2/epub-chapters/?epub_identifier=...,
        # DRF-paginated via a `next` URL. Follow it and adapt each entry.
        url = self.book_info.get("_chapters_url")
        chapters = []
        while url:
            ...
            for c in data["results"]:
                chapters.append(self.adapt_chapter(c))
        ...
        if len(chapters) > sys.getrecursionlimit():
            sys.setrecursionlimit(len(chapters))

        # keep the original behaviour of pulling cover chapter(s) to the front
        cover = [c for c in chapters if "cover" in c["filename"].lower() or "cover" in c["title"].lower()]
        for c in cover:
            chapters.remove(c)

        return cover + chapters

    def adapt_chapter(self, c):
        # Translate a v2 epub-chapter object into the v1 chapter shape used by get().
        content_url = c.get("content_url", "") or ""
        if "/files/" in content_url:
            filename = content_url.split("/files/")[-1]
        else:
            filename = (c.get("reference_id", "") or "").split("/")[-1] or "chapter.html"

        files_base = SAFARI_BASE_URL + "/api/v2/epubs/urn:orm:book:%s/files" % self.book_id
        related = c.get("related_assets", {}) or {}

        # v2 gives absolute asset URLs; the v2 code path in get() rebuilds them as
        # files_base + '/' + <relative>, so strip back to the path under /files/.
        images = []
        for u in related.get("images", []):
            images.append(u.split("/files/")[-1] if "/files/" in u else u)

        stylesheets = [{"url": u} for u in related.get("stylesheets", [])]
    ...
    def adapt_toc(self, nodes):
        # Translate v2 table-of-contents nodes into the shape parse_toc expects.
        out = []
        for n in nodes:
            ref = n.get("reference_id", "") or ""
            href = ref.split("/")[-1] if ref else (n.get("url", "") or "").rstrip("/").split("/")[-1]
            out.append({
                "depth": n.get("depth", 1),
                "fragment": n.get("fragment") or "",
                "id": ref,
                "label": n.get("title", "") or "",
                "href": href,
                "children": self.adapt_toc(n.get("children", []) or []),
            })
        return out

    def create_toc(self):
        response = self.requests_provider(self.book_info.get("_toc_url"))
        if response == 0:
            self.display.exit("API: unable to retrieve book chapters. "
        ...
        navmap, _, max_depth = self.parse_toc(self.adapt_toc(response))
